...
void RenderHeroInfo(game_state *GameState, i32 *X, i32 *Y, i32 YOffset, entity *Hero)
{
...
void RenderInventoryItem(game_state *GameState, i32 *X, i32 *Y, inventory_item *Item)
{
    i32 XPos = *X - WND_WIDTH*0.5f;
    i32 YPos = *Y - WND_HEIGHT*0.5f;
    i32 Width = (WND_WIDTH*0.5f)*0.8f;
...
void UpdateAndRenderInventory(game_state *GameState)
{
    ...
    for(i32 Index = 0;
        ...
        ++Index)
    {
        RenderHeroInfo(GameState, &XPos, &YPos, 10, &GameState->Entities[Index]);
    }
    ...
    for(i32 Index = 0;
        ...
        ++Index)
    {
        RenderInventoryItem(GameState, &XPos, &YPos, &GameState->Inventory.Items[Index]);
    }
}
